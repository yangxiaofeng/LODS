...
optimizer = torch.optim.AdamW(
    [
        ...
        {"params": guidance.learnable_embedding, "lr": config["lr"]["guidance"]},
    ],
    # lr=3e-2,
    weight_decay=0,
)
...
out_dir = os.path.join(
    "outputs", "2d_lods_embedding", f"{config['prompt_processor']['prompt']}{timestamp}"
)
...
for step in tqdm(range(num_steps * n_accumulation_steps + 1)):
    ...
    loss = (loss_dict["loss_sds"] + loss_dict["loss_embedding"]) / n_accumulation_steps
    loss.backward()
